...
Now consider \(\mathcal C \cap [M_1, M_2] = \set {S \cap [M_1, M_2] : S \in \mathcal C}\). This is an open cover of \([M_1,M_2]\). From the compactness of \([M_1,M_2]\), we have a finite cover \(\set {S_1 \cap [M_1, M_2], \ldots, S_n \cap [M_1, M_2]}\) of \([M_1, M_2]\). Since \(\overline \R = [-\infty, M_1) \cup [M_1, M_2] \cup (M_2, \infty]\), \(\set {S_1, \ldots, S_n, U_\infty, U_{-\infty}}\) is then a subcover of \(\mathcal C\) of size at most\footnote{Since one or more of the \(S_i\)s may be equal to \(U_\infty\) or \(U_{-\infty}\).} \(n + 2 < \infty\). Since the cover \(\mathcal C\) was arbitrary we have that \(\overline \R\) is compact.  
\end{proof}
